    ...
    <div class="deznav-scroll">
        <ul class="metismenu" id="menu">
            ...
            @if(
                ...
                || $user->hasPermission('yonetim_siparis_raporu','index')
            )
            <li @class(['mm-active' => request()->is('ust-yonetim-rapor*')])>
                ...
                <ul aria-expanded="false">
                    @if($user->hasPermission('yonetim_kurulu_rapor','index') || $user->hasPermission('yonetim_mudur_giris_cikis_raporu','index'))
                    ...
                    @endif
                    @if($user->hasPermission('yonetim_kurulu_rapor','index') || $user->hasPermission('yonetim_teknik_ariza_raporu','index'))
                    ...
                    @endif
                    @if($user->hasPermission('yonetim_kurulu_rapor','index') || $user->hasPermission('yonetim_servis_raporu','index'))
            ...
            <li @class(['mm-active' => request()->is('servis-planlayici*') || request()->is('servis-takip/planlayici*')])>
                <a href="{{ route('service-planner.index') }}" class="ai-icon" aria-expanded="false">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"
                         fill="none" stroke="currentColor" stroke-width="2"
                         stroke-linecap="round" stroke-linejoin="round" style="min-width:20px">
                        <path d="M3 6h18"></path>
                        ...
                        <path d="M5 6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V8a2 2 0 0 0-2-2"></path>
                    </svg>
                    <span class="nav-text">Servis Planlayici</span>
                </a>
            </li>
            @endif
            ...
            <li @class(['mm-active' => request()->is('servis-takip*')])>
                ...
                <ul aria-expanded="false">
                    ...
                    @endif
                    @if(($user->isSuperAdmin() || $user->isHumanResources()) && $user->hasPermission('shuttle_reports','index'))
                    <li @class(['mm-active' => request()->is('servis-takip/raporlar*')])>
                        <a href="{{ route('shuttle.reports.index') }}">Servis Raporları</a>
                    </li>
                    @endif
                    @if(($user->isSuperAdmin() || $user->isHumanResources()) && $user->hasPermission('shuttle_routes','index'))
                    <li @class(['mm-active' => request()->is('servis-takip/guzergahlar*')])>
                        <a href="{{ route('shuttle.routes.index') }}">Güzergah Tanımları</a>
                    </li>
                    @endif
                    @if(($user->isSuperAdmin() || $user->isHumanResources()) && $user->hasPermission('shuttle_vehicles','index'))
            ...
            <li @class(['mm-active' => request()->is('egitim-ve-gelisim*')])>
                ...
                <ul aria-expanded="false">
                    ...
                    <li @class(['mm-active' => request()->is('egitim-ve-gelisim/yuz-yuze*')])>
                        <a href="{{ route('education.events.index') }}" class="education-sidebar-link">
                            Yuz Yuze Egitimler
                            @if($educationEventPendingCount > 0)
                                <span class="education-sidebar-badge education-sidebar-badge-warning">{{ $educationEventPendingCount }}</span>
                            @endif
                        </a>
                    </li>
                    @endif
            ...
            {{-- BİLGİ İŞLEM --}}
            @if($user->hasPermission('it_computers','index') || $user->hasPermission('it_backup','index') || $user->hasPermission('login_logs','index') || $user->hasPermission('it_agent_inventory','index') || $user->hasPermission('mikrotik','index'))
            <li @class(['mm-active' => request()->is('bilgi-islem*')])>
                ...
                <ul aria-expanded="false">
                    ...
                    @endif
                    @if($user->hasPermission('mikrotik','index'))
                    <li @class(['mm-active' => request()->is('bilgi-islem/mikrotik*')])>
                        <a href="{{ route('it.mikrotik.index') }}">MikroTik</a>
                    </li>
                    @endif
                </ul>
            </li>
