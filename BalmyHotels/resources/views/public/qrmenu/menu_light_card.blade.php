    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cat-image-wrap {
            ...
            padding-top: 42%; overflow: hidden;
            background: var(--bg2);
        }
        .cat-image-wrap img {
        ...
        .cat-image-overlay {
            position: absolute; inset: 0;
            background: linear-gradient(to top, rgba(46,35,24,.40) 0%, transparent 55%);
        }
        ...
        .item-card:hover, .item-card:active {
            box-shadow: 0 6px 24px rgba(46,35,24,.10);
        ...
        .ic-no-img {
            position: absolute; inset: 0;
            display: flex; align-items: center; justify-content: center;
            font-family: var(--cormorant); font-size: 1.6rem;
            color: var(--border2); background: var(--bg2);
        }
        .ic-detail-hint {
            position: absolute; bottom: .45rem; right: .45rem;
            width: 20px; height: 20px; border-radius: 50%;
            background: rgba(253,250,246,.92); border: 1px solid var(--border2);
            display: flex; align-items: center; justify-content: center;
            font-family: var(--jost); font-size: .6rem; font-weight: 500;
            color: var(--accent);
        }
        ...
        .ic-badges { display: flex; gap: .18rem; flex-wrap: wrap; }
        .badge-pill {
        ...
        .ic-allergens { display: flex; gap: .14rem; flex-wrap: wrap; }
        .allergen-chip {
        ...
        .ic-price {
            ...
            color: var(--accent); margin-top: auto; padding-top: .3rem;
            letter-spacing: .02em;
        }
        .ic-glass-bottle { display: flex; flex-direction: column; gap: .1rem; }
        .glass-btl-tag {
        ...
        .footer-rule { display: flex; align-items: center; gap: .55rem; width: 120px; }
        ...
        .footer-text { font-family: var(--jost); font-size: .55rem; letter-spacing: .22em; text-transform: uppercase; color: var(--muted); }
        ...
        .sheet-img-overlay {
            position: absolute; inset: 0;
            background: linear-gradient(to top, rgba(253,250,246,.55) 0%, transparent 50%);
        }
        .sheet-no-img {
            padding: 2rem 0 1.2rem;
            display: flex; flex-direction: column; align-items: center; gap: .5rem;
        }
        .sheet-no-img-rule { display: flex; align-items: center; gap: .45rem; width: 80px; }
        ...
        .sheet-title { font-family: var(--cormorant); font-size: clamp(1.35rem, 5vw, 1.75rem); font-weight: 400; color: var(--text); line-height: 1.15; margin-bottom: .55rem; }
        .sheet-desc { font-family: var(--cormorant); font-style: italic; font-size: .92rem; color: var(--text-sub); line-height: 1.65; margin-bottom: 1rem; }
        .sheet-price-row { display: flex; align-items: baseline; gap: .55rem; margin-bottom: .65rem; padding: .55rem .85rem; background: var(--bg2); border-radius: 8px; border: 1px solid var(--border2); }
        ...
        .glass-btl-sheet-tag { display: flex; justify-content: space-between; align-items: center; background: var(--bg2); border: 1px solid var(--border2); border-radius: 7px; padding: .4rem .7rem; }
        ...
        .sheet-tabs { display: flex; border-bottom: 1px solid var(--border); margin-bottom: .8rem; }
        .sheet-tab-btn { flex: 1; padding: .5rem .4rem; font-family: var(--jost); font-size: .65rem; font-weight: 400; letter-spacing: .1em; text-transform: uppercase; color: var(--muted); background: none; border: none; border-bottom: 1.5px solid transparent; cursor: pointer; transition: color .18s, border-color .18s; }
        ...
        .sheet-options-title { font-family: var(--jost); font-size: .62rem; font-weight: 500; letter-spacing: .12em; text-transform: uppercase; color: var(--muted); margin-bottom: .5rem; border-bottom: 1px solid var(--border); padding-bottom: .4rem; }
        ...
        .allergen-chip-lg { display: inline-flex; align-items: center; gap: .35rem; margin: .2rem .22rem; padding: .3rem .68rem; border-radius: 40px; background: rgba(193,155,119,.1); border: 1px solid rgba(193,155,119,.3); font-family: var(--jost); font-size: .77rem; color: #9a7555; }
        .nut-cell { background: var(--bg2); border-radius: 8px; padding: .45rem .3rem; border: 1px solid var(--border2); text-align: center; }
...
<main class="page-body">
    ...
    <div class="cat-block" id="section-featured">
        ...
        <div class="items-grid">
            ...
            <div class="item-card" onclick="openSheet({{ $fitem->id }})">
                <div class="ic-img-wrap">
                    @if($fitem->image)
                        <img src="{{ asset('uploads/'.$fitem->image) }}" alt="{{ $fitem->getTitle($lang) }}">
                    @else
                        <div class="ic-no-img">✦</div>
                    @endif
                </div>
                <div class="ic-body">
                    <div class="ic-name">{{ $fitem->getTitle($lang) }}</div>
                    @if($fitem->getDescription($lang))
                        <div class="ic-desc">{{ $fitem->getDescription($lang) }}</div>
                    @endif
                    @if($fitem->effectivePrice())
                        <div class="ic-price">{{ $fitem->formattedPrice($menu->currency_symbol) }}</div>
                    @endif
                </div>
            </div>
            @endforeach
            ...
            <div class="item-card" onclick="openSheet({{ $item->id }})">
                <div class="ic-img-wrap">
                    ...
                    @else
                        <div class="ic-no-img">✦</div>
                    @endif
                    @if($hasOpts)<div class="ic-detail-hint">i</div>@endif
                </div>
                <div class="ic-body">
                    ...
                    <div class="ic-allergens">
                        @foreach(array_slice($itemAllergens, 0, 4) as $ak)
                        ...
                        @endif
                        @endforeach
                        @if(count($itemAllergens) > 4)<span class="allergen-chip">+{{ count($itemAllergens) - 4 }}</span>@endif
                    </div>
                    @endif
                    ...
                    <div class="ic-glass-bottle">
                        @if($_pg)<span class="glass-btl-tag">🥃{{ $_cg ? ' '.$_cg.'cl' : '' }} {{ $menu->currency_symbol }}{{ number_format($_pg, 2) }}</span>@endif
                        @if($_pb)<span class="glass-btl-tag">🍾{{ $_cb ? ' '.$_cb.'cl' : '' }} {{ $menu->currency_symbol }}{{ number_format($_pb, 2) }}</span>@endif
                    </div>
                    @endif
                </div>
            </div>
...
<div class="sheet-backdrop" id="sheetBackdrop" onclick="closeSheet(event)">
    <div class="sheet" id="detailSheet">
        ...
        <div id="sheetNoImg" class="sheet-no-img" style="display:none">
...
foreach ($featured as $_fit) {
    ...
    if (!$exists) {
        ...
        $_menuItemsJson[] = [
            ...
            'image' => $_fit->image ? asset('uploads/'.$_fit->image) : null,
...
function openSheet(itemId){
    ...
    var badgesEl=document.getElementById('sheetBadges'); badgesEl.innerHTML='';
    ...
    if(item.nutrition){ var n=item.nutrition; function nutCell(id,val,unit,lbl){ var el=document.getElementById(id); el.innerHTML='<div class="nut-val">'+(val?Math.round(val):'—')+'</div><div class="nut-unit">'+unit+'</div><div class="nut-lbl">'+lbl+'</div>'; } nutCell('nutCalories',n.calories,'kcal',I18N.calories); nutCell('nutProtein',n.protein,'g',I18N.protein); nutCell('nutCarbs',n.carbs,'g',I18N.carbs); nutCell('nutFat',n.fat,'g',I18N.fat); nutWrap.style.display=''; } else { nutWrap.style.display='none'; }
    ...
    if(hasContent){ sheetTabs.style.display=''; sheetTabs.querySelectorAll('.sheet-tab-btn').forEach(function(btn,i){ btn.classList.toggle('active',i===0); }); tabInfo.classList.add('active'); tabIngredients.classList.remove('active'); tabAllergens.classList.remove('active'); sheetTabs.querySelectorAll('.sheet-tab-btn').forEach(function(btn){ btn.onclick=function(){ sheetTabs.querySelectorAll('.sheet-tab-btn').forEach(function(b){ b.classList.remove('active'); }); btn.classList.add('active'); tabInfo.classList.remove('active'); tabIngredients.classList.remove('active'); tabAllergens.classList.remove('active'); if(btn.dataset.tab==='allergens') tabAllergens.classList.add('active'); else if(btn.dataset.tab==='ingredients') tabIngredients.classList.add('active'); else tabInfo.classList.add('active'); }; }); } else { sheetTabs.style.display='none'; tabInfo.classList.add('active'); tabIngredients.classList.remove('active'); tabAllergens.classList.remove('active'); }
