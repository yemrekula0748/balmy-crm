...
@push('styles')
<style>
/* â”€â”€ Sayfa header â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.stats-page-header {
...
/* â”€â”€ DÃ¶nem filtresi â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.period-filter-bar {
    background: #fff;
    border-radius: 12px;
...
/* â”€â”€ Ã–zet kartlar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.summary-stat-card {
    border: none;
    border-radius: 14px;
...
/* â”€â”€ Genel kart â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
.stats-card {
    border: none;
    border-radius: 14px;
...
/* â”€â”€ Tablolar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
<div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h4 class="mb-1 fw-bold">
                    <i class="fas fa-chart-bar me-2" style="color:#f97316"></i>Ä°statistikler & Skor Tablosu
                </h4>
                <ol class="breadcrumb mb-0" style="background:transparent;padding:0;">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">Anasayfa</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('faults.index') }}">Teknik ArÄ±za</a></li>
                    <li class="breadcrumb-item active">Ä°statistikler</li>
                </ol>
            </div>
            <a href="{{ route('faults.index') }}" class="btn btn-outline-light fw-semibold px-4">
                <i class="fas fa-arrow-left me-2"></i>ArÄ±za Listesi
            </a>
        </div>
    </div>
    ...
    <div class="period-filter-bar">
        <form method="GET" class="d-flex align-items-center gap-2 flex-wrap">
            <span class="small fw-semibold text-muted me-1">DÃ¶nem:</span>
            @foreach(['7'=>'Son 7 GÃ¼n','30'=>'Son 30 GÃ¼n','90'=>'Son 90 GÃ¼n','365'=>'Son 1 YÄ±l','all'=>'TÃ¼m Zamanlar'] as $val=>$lbl)
    ...
    <div class="row g-3 mb-4">
        @php
        $summaryCards = [
            ['label'=>'Toplam ArÄ±za',   'value'=>$summary['total'],                                          'color'=>'#4361ee','bg'=>'#eef0ff','icon'=>'fa-clipboard-list'],
            ['label'=>'AÃ§Ä±k',           'value'=>$summary['open'],                                           'color'=>'#dc3545','bg'=>'#fdecea','icon'=>'fa-exclamation-circle'],
            ['label'=>'Ä°ÅŸlemde',        'value'=>$summary['in_progress'],                                    'color'=>'#f97316','bg'=>'#fff3e0','icon'=>'fa-tools'],
            ['label'=>'KapalÄ±',         'value'=>$summary['closed'],                                         'color'=>'#10b981','bg'=>'#e8f5e9','icon'=>'fa-check-double'],
            ['label'=>'Ort. Ã‡Ã¶zÃ¼m (sa)','value'=>$summary['avg_hours'] ?? 'â€”',                              'color'=>'#8b5cf6','bg'=>'#f5f0ff','icon'=>'fa-clock'],
            ['label'=>'SLA Uyumu',      'value'=>($summary['sla_pct'] !== null ? '%'.$summary['sla_pct'] : 'â€”'),'color'=>'#0ea5e9','bg'=>'#e0f2fe','icon'=>'fa-shield-alt'],
        ];
        @endphp
            ...
            <table class="table stats-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">AÃ§Ä±k</th>
                        <th class="text-center">Ä°ÅŸlemde</th>
                        <th class="text-center">KapalÄ±</th>
                        <th class="text-center">Ort. SÃ¼re (sa)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="fw-semibold">{{ $row['dept']?->name ?? 'â€”' }}</div>
                            @if($row['dept']?->branch)
                        ...
                        <td>
                            @if($sla !== null)
                                ...
                                <div class="text-muted" style="font-size:0.68rem">{{ $row['sla_count'] }} Ã¶lÃ§Ã¼m</div>
                            @else
                    ...
                    @empty
                    <tr><td colspan="8" class="text-center text-muted py-4">KayÄ±t bulunamadÄ±.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="col-lg-7">
            <div class="stats-card card h-100">
                <div class="stats-card-header" style="background:linear-gradient(135deg,#f97316,#ea6c0a);color:#fff">
                    <i class="fas fa-tags"></i> ArÄ±za TÃ¼rÃ¼ PerformansÄ±
                </div>
                <div class="table-responsive">
                    <table class="table stats-table mb-0">
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else<span class="text-muted small">â€”</span>@endif
                                </td>
                            </tr>
                            @empty
        ...
        <div class="col-lg-5 d-flex flex-column gap-4">
            ...
            <div class="stats-card card">
                <div class="stats-card-header" style="background:linear-gradient(135deg,#10b981,#059669);color:#fff">
                    <i class="fas fa-map-marker-alt"></i> Konum BazlÄ± (Top 10)
                </div>
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <div class="d-flex gap-1">
                                @if($row['open'] > 0)
                                    <span class="badge" style="background:#fdecea;color:#dc3545;font-size:0.68rem">{{ $row['open'] }} aÃ§Ä±k</span>
                                @endif
        ...
        <div class="col-{{ $branchStats ? 'lg-8' : '12' }}">
            <div class="stats-card card">
                <div class="stats-card-header" style="background:#fff;border-bottom:1px solid #e9ecef;color:#344054">
                    <i class="fas fa-chart-line text-primary"></i> AylÄ±k ArÄ±za Trendi
                </div>
        ...
        <div class="col-lg-4">
            <div class="stats-card card h-100">
                <div class="stats-card-header" style="background:linear-gradient(135deg,#0ea5e9,#0284c7);color:#fff">
                    <i class="fas fa-building"></i> Åube KarÅŸÄ±laÅŸtÄ±rmasÄ±
                </div>
                <div class="card-body p-3">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-2">
                            <span class="badge" style="background:#fdecea;color:#dc3545;font-size:0.68rem">{{ $row['open'] }} aÃ§Ä±k</span>
                            <span class="badge" style="background:#e8f5e9;color:#10b981;font-size:0.68rem">{{ $row['closed'] }} kapalÄ±</span>
                        </div>
                    </div>
                    @endforeach
                </div>
