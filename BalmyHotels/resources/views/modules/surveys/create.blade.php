...
<div class="container-fluid">
    <div class="row page-titles mx-0">
        <div class="col-sm-6 p-md-0">
            <div class="welcome-text"><h4><i class="fas fa-plus-circle me-2 text-primary"></i>Yeni Anket Oluştur</h4></div>
        </div>
        ...
        <div class="row g-3 align-items-start">
            ...
            <div class="col-xl-4" style="position:sticky;top:1rem">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-check form-switch mb-0">
                            <input class="form-check-input" type="checkbox" name="show_language_select" id="showLang" value="1" checked>
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        <p class="text-muted small mb-3">En az bir dil seçin. Her eklenen dil için anket başlığı ve sorular ayrıca girilecek.</p>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div id="questionsContainer">
                            {{-- Sorular JS ile eklenir --}}
                        </div>
                        <div id="emptyMsg" class="text-center text-muted py-5">
                            <i class="fas fa-question-circle fa-3x opacity-25 mb-3 d-block"></i>
                            Henüz soru eklenmedi. "Soru Ekle" butonuna tıklayın.
                        </div>
                    </div>
                    <div class="card-footer d-flex gap-2 justify-content-end">
                        <a href="{{ route('surveys.index') }}" class="btn btn-secondary">İptal</a>
...
let qCounter     = 0;
const titleState = {}; // lang → { title, desc }  — başlık yeniden render'da kaybolmasın
const activeQTab = {}; // tempId → lang          — soru dil tabı korunsun
...
document.addEventListener('DOMContentLoaded', () => {
    // Initial languages from checked checkboxes
    readLanguages();
    renderTitleTabs();
    renderAllQuestions();
});
...
function renderTitleTabs() {
    saveTitleState(); // önce mevcut değleri kaydet
    ...
    languages.forEach((lang, i) => {
        ...
        const savedTitle = titleState[lang]?.title ?? OLD_TITLE[lang] ?? '';
        const savedDesc  = titleState[lang]?.desc  ?? OLD_DESC[lang]  ?? '';
        ...
        content += `<div class="tab-pane fade ${show}" id="title_tab_${lang}">
                        <div class="mb-2">
                            ...
                            <input type="text" name="title[${lang}]" class="form-control form-control-sm"
                                   placeholder="${info.name} dilinde başlık..."
                                   value="${escHtml(savedTitle)}"
...
function renderAllQuestions() {
    ...
    // Aktif dil tab'ını kaydet (yeniden render'da sıfırlanmasın)
    ...
    questions.forEach(q => {
        ...
        if (!card) return;
        const oldActive  = card.querySelector('.nav-link.active');
        const newActive  = card.querySelector(`[href="#q_${q.temp_id}_${saved}"]`);
        const oldPane    = card.querySelector('.tab-pane.active');
        const newPane    = card.querySelector(`#q_${q.temp_id}_${saved}`);
...
function renderQuestionCard(q, index) {
    ...
    // Language tabs for this question
    let langTabs = '';
    let langContent = '';
    ...
    // Conditional dropdown — only show questions before this one that have options
    ...
    if (prevWithOptions.length > 0) {
        ...
        <div class="border rounded p-2 mt-2 bg-light">
            <div class="form-check form-switch mb-2">
                ...
                <label class="form-check-label small" for="cond_enable_${q.temp_id}">
                    <i class="fas fa-code-branch me-1"></i>Koşullu göster
                </label>
            </div>
            <div id="cond_panel_${q.temp_id}" style="display:${q.conditional_temp_id?'block':'none'}">
                <select class="form-select form-select-sm mb-2" onchange="setCondQuestion('${q.temp_id}',this.value)">
                    ...
                    ${prevWithOptions.map(pq => `<option value="${pq.temp_id}" ${pq.temp_id===selQ?'selected':''}>${index>0?questions.indexOf(pq)+1+'. ':''} ${escHtml(pq.translations[languages[0]]?.text||'Soru '+(questions.indexOf(pq)+1))}</option>`).join('')}
                </select>
                <input type="text" class="form-control form-control-sm" placeholder="Göster eğer cevap şuysa..."
                       value="${escHtml(selV)}"
                       oninput="setCondValue('${q.temp_id}',this.value)">
                <small class="text-muted">Seçilen soruya verilen cevap bu değer olduğunda bu soru görünür.</small>
            </div>
        </div>`;
    }

    return `
    <div class="card mb-3 border question-card" id="qcard_${q.temp_id}">
        <div class="card-header py-2 d-flex align-items-center gap-2" style="background:#f8f9fc">
            ...
            <select class="form-select form-select-sm" style="max-width:220px" onchange="setQuestionType('${q.temp_id}',this.value)">
                ${Object.entries(QUESTION_TYPES).map(([val, info]) =>
                    `<option value="${val}" ${q.type===val?'selected':''}><i class="fas ${info.icon}"></i> ${info.label}</option>`
                ).join('')}
            </select>
            <div class="form-check form-switch ms-2 mb-0">
                <input class="form-check-input" type="checkbox" id="req_${q.temp_id}"
                       ${q.is_required?'checked':''}
                       onchange="setRequired('${q.temp_id}',this.checked)">
                <label class="form-check-label small" for="req_${q.temp_id}">Zorunlu</label>
            </div>
            <button type="button" class="btn btn-sm btn-outline-danger ms-auto" onclick="removeQuestion('${q.temp_id}')">
...
function renderOptionsEditor(q, lang) {
    ...
    let html = `<div class="options-editor" id="opts_${q.temp_id}_${lang}">`;
    ...
    html += `<button type="button" class="btn btn-sm btn-outline-primary mt-1" onclick="addOption('${q.temp_id}','${lang}')">
...
// MUTATION HELPERS
// ================================================================
function getQuestion(tempId) { return questions.find(q => q.temp_id === tempId); }

function setQuestionType(tempId, type) {
    const q = getQuestion(tempId);
    if (!q) return;
    q.type = type;
    renderAllQuestions();
}

function setRequired(tempId, val) {
    const q = getQuestion(tempId);
    if (q) q.is_required = val;
}

function setQuestionText(tempId, lang, val) {
    const q = getQuestion(tempId);
    if (!q) return;
    if (!q.translations[lang]) q.translations[lang] = { text: '', options: [] };
    q.translations[lang].text = val;
}

function addOption(tempId, lang) {
    const q = getQuestion(tempId);
    if (!q) return;
    if (!q.translations[lang]) q.translations[lang] = { text: '', options: [] };
    if (!q.translations[lang].options) q.translations[lang].options = [];
    q.translations[lang].options.push('');
    renderAllQuestions();
}

function removeOption(tempId, lang, idx) {
    const q = getQuestion(tempId);
    if (!q) return;
    q.translations[lang]?.options?.splice(idx, 1);
    renderAllQuestions();
}

function setOption(tempId, lang, idx, val) {
    const q = getQuestion(tempId);
    if (!q) return;
    if (!q.translations[lang]) q.translations[lang] = { text: '', options: [] };
    if (!q.translations[lang].options) q.translations[lang].options = [];
    q.translations[lang].options[idx] = val;
}

function toggleCondPanel(tempId, show) {
    const panel = document.getElementById('cond_panel_' + tempId);
    if (panel) panel.style.display = show ? 'block' : 'none';
    if (!show) {
        const q = getQuestion(tempId);
        if (q) { q.conditional_temp_id = null; q.conditional_answer_value = null; }
    }
}

function setCondQuestion(tempId, val) {
    const q = getQuestion(tempId);
    if (q) q.conditional_temp_id = val || null;
}

function setCondValue(tempId, val) {
    const q = getQuestion(tempId);
    if (q) q.conditional_answer_value = val || null;
}
...
function prepareSubmit() {
    // Sync any values that might not have triggered oninput
